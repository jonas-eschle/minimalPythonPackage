...
`conda` is a package manager that installs packages from the Anaconda repository, other channels can be added. Install `conda` by downloading and installing `miniforge` from here: https://github.com/conda-forge/miniforge#download. This has the `conda-forge` channel enabled by default.
...
Always create a virtual environment! Best is to use `conda`/`mamba` for this.
